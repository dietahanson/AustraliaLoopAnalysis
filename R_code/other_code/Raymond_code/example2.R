## This is an example of the modelling technique used in:
...
##
## In this example, we take "uncertain" links into account by iterating over all possible combinations
##  of uncertain links.
...
unk_idx <- grep("unknown", x$Importance, ignore.case=TRUE)
...
for (k in (1:dim(x)[1])) {
  if (grepl("minor", x[k, ]$Importance, ignore.case=TRUE)) {
    next
  }
  this_importance <- as.character(x[k, ]$Importance)
  if (grepl("unknown", this_importance, ignore.case=TRUE)) {
    if (! k %in% unk_idx) {
...
barplot(rsummary_cm, cex.names=0.7, las=2, legend=TRUE, xpd=TRUE, args.legend=list(horiz=TRUE, y=1.25), ylab="Proportion")
...
## all simulation runs
par(mfrow=c(2, 1), fig=c(0.02, 0.98, 0.2, 0.8))
barplot(rsummary_pcm, cex.names=0.7, las=2, legend=TRUE, xpd=TRUE, args.legend=list(horiz=TRUE, y=1.25), ylab="Proportion")
title("Eradication project predictions", line=3)

## also show only those in which the target species were actually suppressed
par(mfrow=c(2, 1), fig=c(0.02, 0.98, 0.2, 0.8))
barplot(rsummary_pcm_subset, cex.names=0.7, las=2, legend=TRUE, xpd=TRUE, args.legend=list(horiz=TRUE, y=1.25), ylab="Proportion")
...
## print results
cat("Frequency of outcomes: which combinations of species were successfully suppressed, how often?")
print(tempoutcome)
